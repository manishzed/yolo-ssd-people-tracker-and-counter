...
skip_frame = 28 #24 
face_age_gen ={}
face_age_gen__ =defaultdict(list)
object_id_list = []
...
while True:
    ...
    for (objectId, bbox) in objects.items():
        ...
        #frame time ----end--------
        if objectId in face_age_gen.keys():
            values=face_age_gen[objectId]
            #print("values:", values)
            if values:
                age1 =values[0]
                gender1 =values[1]
                
                #print("id, age, gender:",objectId, age1,gender1,date1)
            
                # Add text
                #cv2.putText(img, f'{gender1}, {age1}', (x1, y1-20), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,0), 2)
                cv2.putText(frameOpencvDnn, f'{gender1}, {age1}', (x22-120, y22+30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,0), 2)
                #cv2.putText(frameOpencvDnn, f'{gender1}, {age1}', (x11, y11-20), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 0, 255), 1)
        ...
        if frame_counter % skip_frame == 0:
            ...
            #print("id:{}, age:{}, gender:{}, fc:{}".format(objectId, age, gender, frame_counter))
            face_age_gen__[objectId].append([age, gender, frame_counter])
            print("face_age_gen__:", face_age_gen__)
            
            
            if objectId in face_age_gen__.keys():
                if len(face_age_gen__[objectId])>=2:
                    print("xyz:",face_age_gen__)
                    #first frame
                    
                    if objectId not in object_id_list:
                        object_id_list.append(objectId)
                        
                        face_age_gen[objectId] =[age, gender, frame_counter]
                        print("1111:", face_age_gen__)
                        print("222:", face_age_gen)
# =============================================================================
#                         
#             if objectId not in object_id_list:
#                 object_id_list.append(objectId)
...
#                 face_age_gen[objectId] =[age, gender, frame_counter]
# =============================================================================
            #face_age_gen[objectId] =[age, gender]

    #print("face_age_gen22222222:", face_age_gen)
    ##fps
